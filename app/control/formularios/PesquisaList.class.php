<?php
/**
 * PesquisaList Listing
 * @author  <your name here>
 */
class PesquisaList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $this->form->setData( TSession::getValue('Pesquisa_filter_data') );
        
        // add the search form actions
        $btn = $this->form->addAction(_t('Find'), new TAction([$this, 'onSearch']), 'fa:search');
        $btn->class = 'btn btn-sm btn-primary';
        $this->form->addActionLink(_t('New'), new TAction(['PesquisaForm', 'onEdit']), 'fa:plus green');
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('PesquisaList_filter_nome',   NULL);
        TSession::setValue('PesquisaList_filter_data',   NULL);

        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'like', "%{$data->nome}%"); // create the filter
            TSession::setValue('PesquisaList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->data) AND ($data->data)) {
            $filter = new TFilter('data', 'like', "%{$data->data}%"); // create the filter
            TSession::setValue('PesquisaList_filter_data',   $filter); // stores the filter in the session
        }
        ...
        TSession::setValue('Pesquisa_filter_data', $data);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Pesquisa
            $repository = new TRepository('Pesquisa');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('PesquisaList_filter_nome')) {
                $criteria->add(TSession::getValue('PesquisaList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('PesquisaList_filter_data')) {
                $criteria->add(TSession::getValue('PesquisaList_filter_data')); // add the session filter
            }
    ...
    public static function onDelete($param)
    {
        ...
        TTransaction::open('procon_com');
        $criteria = new TCriteria;
        $criteria->add(new TFilter('pesquisa_id', '=', $param['id']));
        $result = $repository->load($criteria);
        TTransaction::close();
        if(empty($result)){
            new TQuestion(TAdiantiCoreTranslator::translate('Do you really want to delete ?'), $action);
        } else {
            new TQuestion(('A exclusão dessa pesquisa removerá todas as Relações (preços) associados a mesma. Deseja Continuar?'), $action);
        }
    }
    ...
    public function onDeleteCollection( $param )
    {
        ...
        if ($data)
        {
            ...
            if ($selected)
            {
                ...
                TTransaction::close();
                if($dependencia){
                    new TQuestion(('A exclusão dessas pesquisas removerá todas as Relações (preços) associados as mesmas. Deseja Continuar?'), $action);
                } else{
                    new TQuestion(TAdiantiCoreTranslator::translate('Do you really want to delete ?'), $action);
                }
            }
