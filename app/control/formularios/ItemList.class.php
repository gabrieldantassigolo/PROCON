...
class ItemList extends TPage
{
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('ItemList_filter_id',   NULL);
        TSession::setValue('ItemList_filter_nome',   NULL);
        TSession::setValue('ItemList_filter_quantidade',   NULL);

        if (isset($data->id) AND ($data->id)) {
            $filter = new TFilter('id', '=', "$data->id"); // create the filter
            TSession::setValue('ItemList_filter_id',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('ItemList_filter_id')) {
                $criteria->add(TSession::getValue('ItemList_filter_id')); // add the session filter
            }
